...
def find_top_pixels(total_diff: np.array, top_n: int): 
    ...
    top_coords = [np.unravel_index(idx, total_diff.shape) for idx in top_indices]
...
for original, idx, timestamp in zip(image_filenames,np.arange(1, len(image_filenames)), timestamps):
    ...
    cut_original, cut_mask = cutting(original)
    smoothed_img = smoothing(original)
    cut_smoothed, cut_mask = cutting(smoothed_img)
    rgb_difference = rgb_calc(cut_original, cut_smoothed, cut_mask)
